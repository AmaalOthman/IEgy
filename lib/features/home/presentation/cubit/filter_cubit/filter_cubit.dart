...
class FilterCubit extends Cubit<FilterState> {
  ...
  List<bool> categorySelection = [true, false, false, false, false, false, false, false, false];

  List<bool> colorSelection = [true, false, false, false, false, false, false, false, false];
  ...
  void onColorPressed(int index) {
    for(int i = 0; i < colorSelection.length; i++)
      colorSelection[i] = false;
    colorSelection[index] = true;
    emit(SelectColorState());
  }

  void applyFiltration(BuildContext context) {
    showToast(message: AppLocalizations.of(context)!.filtration_applied, state: ToastStates.success);
    Navigator.pop(context);
  }
